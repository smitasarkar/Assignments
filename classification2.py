...
class classification():
     def __init__(self,path='ml_data.csv',clf_opt='lr',no_of_selected_features=None):
        self.path = path
...
# Load the data 
    ...
    # Load the file using Pandas       
        reader=pd.read_csv(self.path+'ml_data.csv')  
    ...
    # Select all rows except the ones belong to particular class'
        ...
        training_data1, validation_data, training_cat1, validation_cat = train_test_split(data, labels, test_size=0.05, random_state=42,stratify=labels)   

        smote = SMOTE(random_state=42)
        training_data, training_cat = smote.fit_resample(training_data1, training_cat1)
