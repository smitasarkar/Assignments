...
class classification():
     def __init__(self,path='/home/rasel/Downloads/MLproject/',clf_opt='lr',no_of_selected_features=None):
        self.path = path
...
# Selection of classifiers  
    ...
    # AdaBoost 
        if self.clf_opt=='ab':
            ...
            be3 = RandomForestClassifier(max_features=None,class_weight='balanced')
...
# Load the data 
    ...
    # Load the file using Pandas       
        reader=pd.read_csv(self.path+"adasyn_data.csv")  
...
# Classification using the Gold Statndard after creating it from the raw text    
   ...
   # Get the data
        data,labels=self.get_data('adasyn_data.csv')
        data=np.asarray(data)
...
# Experiments using training data only during training phase (dividing it into training and validation set)
    ...
    # Evaluation
        ...
        if confidence_score>0.85:
            ...
            data,tst_cat=self.get_data('adasyn_data.csv')
            tst_data=np.asarray(data)
            predicted=clf.predict(tst_data)
            print('\n ##### Classification Report ##### \n')
            print(classification_report(tst_cat, predicted_cat, target_names=class_names)) 
        else:   
